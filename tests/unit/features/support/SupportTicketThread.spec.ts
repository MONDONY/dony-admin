...
describe('SupportTicketThread', () => {
  ...
  it("affiche l'en-tete du ticket et emet close sur la croix", async () => {
  ...
  describe('assignation', () => {
    ...
    it("propose Reprendre sur le ticket d'un collegue, et reassigne vers soi", async () => {
    ...
    it('masque toutes les actions a un role sans SUPPORT_TICKET_MANAGE', () => {
  ...
  describe('reponse', () => {
    ...
    it('garde le bouton Repondre inerte tant que le brouillon est vide ou blanc', async () => {
      const w = mountThread(mine)
    ...
    it("n'envoie rien pendant qu'une action est deja en cours", async () => {
    ...
    it('emet reply avec les cles des images jointes', async () => {
      const w = mountThread(mine)
    ...
    it("autorise une reponse faite d'une image sans texte", async () => {
      const w = mountThread(mine)
      await w.findComponent({ name: 'SupportAttachmentUploader' })
        .vm.$emit('change', ['support/admin/u1/1_a.jpg'])
    ...
    it("bloque l'envoi pendant un upload", async () => {
      const w = mountThread(mine)
      await w.findComponent({ name: 'SupportAttachmentUploader' }).vm.$emit('busy', true)
      await w.find('textarea').setValue('Voici')
      expect(w.findAll('button').find(b => b.text().includes('Répondre'))!
        .attributes('disabled')).toBeDefined()
    })

    it("masque l'uploader a un role sans SUPPORT_TICKET_MANAGE", () => {
      seedAuth('SUPPORT', { SUPPORT_TICKET_MANAGE: false })
      const w = mountThread(mine)
      expect(w.findComponent({ name: 'SupportAttachmentUploader' }).exists()).toBe(false)
    })

    it("n'offre pas d'uploader sur un ticket resolu", () => {
  ...
  it("affiche l'erreur d'action renvoyee par le serveur", () => {
